...
class Step : Module() {
    ...
    private val modeValue = ListValue("Mode", arrayOf(
            "Vanilla", "Jump", "NCP", "MotionNCP", "OldNCP", "AAC", "LAAC", "AAC3.3.4", "Spartan", "Rewinside", "NewNCP"
    ), "NCP")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when {
            ...
            mode.equals("newncp", true) -> {
                var height = mc.thePlayer.entityBoundingBox.minY - mc.thePlayer.posY


                if (height > 0.6f && height <= 1.0f){
                    val array = floatArrayOf(0.42f, 0.75f)

                    for (value in array) {
                        mc.thePlayer.sendQueue.addToSendQueue(C03PacketPlayer.C04PacketPlayerPosition(mc.thePlayer.posX, mc.thePlayer.posY + value, mc.thePlayer.posZ, false))
                        ++timerSlow;
                    }
                }
            }
